...
pub fn write_packets<W: Write + Seek>(stream: &mut W, packets: &Vec<Packet>, cc: &mut ContinuityCounter) {
  for packet in packets {
    ...
    if fill_count > 0 {
      println!("MpegTS Writer: wrong packet (with PID {}) length {:?}", packet.program_id, end_position - origin_position);
      println!("{:?}", packet);
...
pub fn write_packet<W: Write + Seek>(mut stream: &mut W, packet: &Packet, cc: &mut ContinuityCounter) {
  ...
  writer.write(2, packet.transport_scrambling_control).unwrap();
