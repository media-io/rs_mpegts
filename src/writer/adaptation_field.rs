...
pub fn write(af: &AdaptationField) -> Vec<u8> {
  let mut data = Vec::new();
  {
    ...
    af_writer.write_bit(af.elementary_stream_priority_indicator).unwrap();
    ...
    match af.splice_countdown {
      ...
      Some(ref splice_countdown) => {
        if *splice_countdown < 0 {
          af_writer.write(8, (- splice_countdown) as u8 + 0x80 ).unwrap();
        } else {
...
pub fn write_adaptation_field(mut writer: &mut BitWriter<BigEndian>, adaptation_field: &Option<AdaptationField>) {
